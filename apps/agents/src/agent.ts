...
export class ParallelAgent extends Agent<Env> {
  ...
  async onRequest(request: Request): Promise<Response> {
    ...
    console.log(`[Agent:${this.name}] ${request.method} ${resolvedPathname}`);

    if (resolvedPathname.startsWith("/chat")) {
      return this.#handleChatRequest(request, resolvedPathname, pathnameParts);
    }

    console.warn(`[Agent:${this.name}] Route not found: ${resolvedPathname}`);
  ...
  async onAlarm(): Promise<void> {
    try {
      ...
      console.error(`[Agent:${this.name}] Error in alarm handler:`, error);
  ...
  async onConnect(
    ...
    const resumeConversationId = url.searchParams.get("resumeConversationId");
  ...
  async onMessage(_connection: Connection, message: WSMessage): Promise<void> {
    ...
    // Fetch notebook context from sync server
    const notebookContext = await this.fetchNotebookContext();
    console.log(
      `[Agent:${this.name}] Notebook markdown before chat (${notebookContext?.length ?? 0} chars):`,
      notebookContext ? notebookContext.slice(0, 500) : "(empty)",
    );
    const systemPrompt = notebookContext
      ? `Here is the current notebook context:\n\n<notebook_context>\n${notebookContext}\n</notebook_context>`
      : undefined;
    ...
    await this.#runAgent({
      ...
      onFinish: async (completion) => {
        const finishChunk: Chunk = {
          ...
          totalUsage: completion.totalUsage as
  ...
  async fetchNotebookContext(): Promise<string | null> {
    ...
    try {
      const res = await fetch(
        `${syncUrl}/parties/document/workspace-${this.name}/markdown`,
        {
          headers: {
  ...
  async #generateSummary(
    ...
    try {
      ...
      return (
        summary.text || "Summary generation completed but no text was produced."
      );
    } catch (error) {
